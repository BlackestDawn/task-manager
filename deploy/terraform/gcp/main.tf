...
# Secret containers only — the actual values (DB connection string, JWT
# signing secret) are pushed in separately so they never land in this
# stack's state. DB URL comes from the Neon stack's output; the JWT secret
# is generated by hand (e.g. `openssl rand -base64 32`).
...
resource "google_secret_manager_secret" "jwt_secret" {
  for_each  = toset(local.envs)
  secret_id = "${var.github_repo}-${each.value}-jwt-secret"
